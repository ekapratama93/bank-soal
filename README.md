...
Aplikasi pembuat soal latihan sekolah dengan bantuan AI (OpenRouter). Siswa memilih mata pelajaran + kelas + tipe ujian, aplikasi menyajikan paket soal campuran (pilihan ganda, benar/salah, isian), timer countdown, penilaian otomatis (pilihan ganda/benar-salah dicocokkan langsung; isian & uraian/deskripsi dikoreksi AI, isian dengan fallback pencocokan string lokal bila AI gagal), dan riwayat hasil tersimpan di browser.

**Semua teks UI, soal, dan pembahasan dalam Bahasa Indonesia.**

## Konsep utama

- **Tipe ujian** dikelola admin (Ujian Harian, Ujian Semester, dst.). Tiap tipe bisa mengatur jumlah soal, durasi, **komposisi tipe soal** (jumlah per tipe: pilihan ganda, benar/salah, isian singkat, uraian/deskripsi), dan **poin per tipe soal** (mis. pilihan ganda 2 poin, isian 5 poin, uraian 10 poin; tipe tak disebut = 1 poin). Jika kosong, mengikuti konfigurasi kelas dengan komposisi otomatis dan semua soal bernilai 1 poin. Nilai siswa = poin diperoleh ÷ total poin × 100. Soal isian & uraian/deskripsi dikoreksi & dinilai AI (skor dikali poin tipe) — isian dinilai biner (benar/salah), uraian/deskripsi bisa dapat skor parsial (0–1) sesuai kelengkapan jawaban; khusus isian, kalau panggilan AI gagal dipakai fallback pencocokan string lokal supaya pengumpulan tetap berhasil.
...
- Soal **isian** dikoreksi AI (dinilai dari kesamaan makna, bukan sekadar string persis — jawaban dengan kata berbeda/sinonim/salah ketik kecil tetap bisa dianggap benar), dengan nilai tetap **biner** (benar/salah, tidak ada skor parsial). Kalau panggilan AI untuk isian gagal (API down, rate limit, dll.), otomatis jatuh ke pencocokan string lokal (`backend/internal/textmatch/textmatch.go`, toleransi kecil pada salah ketik/ejaan) supaya pengumpulan tetap berhasil — kegagalan AI tidak pernah menggagalkan soal isian. Soal **uraian/deskripsi** tetap dikoreksi AI dengan skor kontinu 0–1 (boleh parsial) dan tidak punya fallback lokal (jarang dipakai — komposisi otomatis tidak pernah menyertakan deskripsi), jadi kalau AI gagal untuk deskripsi, pengumpulan gagal (502) dan siswa bisa coba lagi.
- Soal isian & uraian/deskripsi dinilai AI dalam batch kecil (`gradeBatchSize` di `backend/internal/llm/grading.go`) yang dijalankan **paralel** (goroutine per batch, dibatasi maksimal beberapa batch bersamaan lewat semaphore, `maxConcurrentBatches`), dengan percobaan ulang otomatis (`gradeMaxAttempts`) baik saat panggilan AI gagal maupun responsnya tidak valid — supaya kegagalan atau lambatnya satu batch tidak menahan atau menggagalkan batch lain, dan waktu tunggu mendekati satu batch terlama, bukan jumlah semua soal digabung.
